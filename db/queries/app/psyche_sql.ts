...
SELECT id, user_id, time_range, valence, vitality, created_at, updated_at FROM vibes WHERE user_id = $1 ORDER BY time_range DESC`;
...
SELECT id, user_id, occurred_at, activating_status, reaction_status, coping_status, post_status, details, created_at, updated_at FROM experiences WHERE user_id = $1 ORDER BY occurred_at DESC`;
...
SELECT id, user_id, time_range, valence, vitality, created_at, updated_at FROM vibes WHERE user_id = $1 AND id = $2`;
...
SELECT id, user_id, occurred_at, activating_status, reaction_status, coping_status, post_status, details, created_at, updated_at FROM experiences WHERE user_id = $1 AND id = $2`;
...
export const updateVibeQuery = `-- name: UpdateVibe :one
UPDATE vibes
...
export interface UpdateVibeArgs {
    userId: string;
    id: string;
    valence: number;
    vitality: number;
...
export interface UpdateVibeRow {
    id: string;
    userId: string;
    timeRange: string;
    valence: number;
...
export async function updateVibe(sql: Sql, args: UpdateVibeArgs): Promise<UpdateVibeRow | null> {
    const rows = await sql.unsafe(updateVibeQuery, [args.userId, args.id, args.valence, args.vitality]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
export async function updateExperience(sql: Sql, args: UpdateExperienceArgs): Promise<UpdateExperienceRow | null> {
    const rows = await sql.unsafe(updateExperienceQuery, [args.userId, args.id, args.details]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
